...
# estimate_var <- FALSE
#
# fit <- ml_logistic_logistic_linear(all_data = all_data,
#                                y_var = "y",
#                                x_var = "x",
#                                z_var = "z",
#                                d_vars = "d",
...
ml_logistic_logistic_linear <- function(all_data = NULL,
                                        main = NULL,
                                        internal = NULL,
                                        external = NULL,
                                        y_var,
  ...
  if (some.m) {
    lf.full <- function(y,
      ...
      dens <- apply(z, 2, function(z) {
        ...
        # P(X=1|Z,C,B)
        p_x.zcb <- (1 + exp(-dcb.term - gamma_z * s))^(-1)

        # f(Y,X,Z|D,C,B) = P(Y=y|X,Z,C,B) P(X=x|Z,D,C,B) f(Z|D,C)
  ...
  llf <- function(f.theta, estimating.hessian = FALSE) {
    ...
    if (some.m) {
      ...
      # L = \int_z f(Y|X,Z,C,B) P(X=x|Z,D,C,B) f(Z|D,C) dZ
    ...
    if (some.i) {
      ...
      # L = P(Y=y|X,Z,C,B) P(X=x|Z,D,C,B) f(Z|D,C)
      ll.i <- sum(
