...
# estimate_var <- TRUE
#
# fit <- ml_linreg_linreg(all_data = all_data,
#                         y_var = "y",
#                         z_var = "z",
#                         d_vars = "d",
#                         c_vars = "c",
...
ml_linreg_linreg <- function(all_data = NULL,
                             main = NULL,
                             internal = NULL,
                             external = NULL,
                             y_var,
  ...
  covariates <- c(z_var, d_vars, c_vars, b_vars)
  ...
  if (! is.null(all_data)) {
    main <- all_data[! is.na(all_data[, y_var]) & is.na(all_data[, z_var]), ]
    internal <- all_data[! is.na(all_data[, y_var]) & ! is.na(all_data[, z_var]), ]
    external <- all_data[is.na(all_data[, y_var]) & ! is.na(all_data[, z_var]), ]
  }
